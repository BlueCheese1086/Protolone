...
public class RobotContainer {
    ...
    public RobotContainer() {
        ...
        // Shooter Sys ID
        autoChooser.addOption("Shooter SysId Quasistatic Forward",
                shooter.getShooterSysIdQuasistatic(Direction.kForward));  // Run For Ks and Kv
                
        autoChooser.addOption("Shooter SysId Quasistatic Reverse",
                shooter.getShooterSysIdQuasistatic(Direction.kReverse));

        autoChooser.addOption("Shooter SysId Dynamic Forward", shooter.getShooterSysIdDynamic(Direction.kForward));
        autoChooser.addOption("Shooter SysId Dynamic Reverse", shooter.getShooterSysIdDynamic(Direction.kReverse));
                ...
                .whileTrue(
                        Commands.run(
                                () -> {
                                    ...
                                    Translation2d lookaheadDelta = AllianceFlipUtil.apply(targetPosition)
                                            .minus(lookahead.getTranslation());
                                ...
                                Commands.race(
                                        DriveCommands.joystickDriveAtAngle(
                                                ...
                                                () -> ShooterSettings.interpolateAngle(
                                                        targetPosition.minus(drive.getPose().getTranslation())
                                                                .getNorm())),
                                        Commands.run(
                                                () -> shooter.shoot(
